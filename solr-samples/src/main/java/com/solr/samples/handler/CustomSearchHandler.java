 ...
 */
package com.solr.samples.handler;
...
/**
 *
 * This is a copy of the OOTB "SearchHandler" for Solr. Intended to be used to create custom search logic
 *
 */
public class CustomSearchHandler extends RequestHandlerBase implements SolrCoreAware , PluginInfoInitialized, PermissionNameProvider {
    ...
    private ShardHandler getAndPrepShardHandler(SolrQueryRequest req, CustomResponseBuilder rb) {
     ...
     */
    protected CustomResponseBuilder newResponseBuilder(SolrQueryRequest req, SolrQueryResponse rsp, List<SearchComponent> components) {
    ...
    public void handleRequestBody(SolrQueryRequest req, SolrQueryResponse rsp) throws Exception
    {
        ...
        CustomResponseBuilder rb = newResponseBuilder(req, rsp, components);
        if (rb.requestInfo != null) {
            rb.requestInfo.setResponseBuilder(rb);
        }

        SolrParams params = req.getParams();
        boolean dbg = params.getBool(CommonParams.DEBUG_QUERY, false);
        // Custom parameter for filtering sample data by author
        String author = params.get("author", "");
        rb.setDebug(dbg);
        if (dbg == false){//if it's true, we are doing everything anyway.
            SolrPluginUtils.getDebugInterests(params.getParams(CommonParams.DEBUG), rb);
        }
                ...
                while (rb.outgoing.size() > 0) {
                    ...
                    while (rb.outgoing.size() > 0) {
                        ...
                        for (String shard : sreq.actualShards) {
                            ModifiableSolrParams modParams = new ModifiableSolrParams(sreq.params);
                            modParams.remove(ShardParams.SHARDS);      // not a top-level request
                            modParams.set(DISTRIB, "false");               // not a top-level request
                            modParams.remove("indent");
                            modParams.remove(CommonParams.HEADER_ECHO_PARAMS);
                            modParams.set(ShardParams.IS_SHARD, true);  // a sub (shard) request
                            modParams.set(ShardParams.SHARDS_PURPOSE, sreq.purpose);
                            modParams.set(ShardParams.SHARD_URL, shard); // so the shard knows what was asked
                            modParams.set(CommonParams.OMIT_HEADER, false);
                            if (rb.requestInfo != null) {
                                // we could try and detect when this is needed, but it could be tricky
                                modParams.set("NOW", Long.toString(rb.requestInfo.getNOW().getTime()));
                            }
                            String shardQt = modParams.get(ShardParams.SHARDS_QT);
                            if (shardQt != null) {
                                modParams.set(CommonParams.QT, shardQt);
                            } else {
                                ...
                                if (!"/select".equals(reqPath)) {
                                    modParams.set(CommonParams.QT, reqPath);
                                } // else if path is /select, then the qt gets passed thru if set
                            }
                            shardHandler1.submit(sreq, shard, modParams);
                        }
                    }
